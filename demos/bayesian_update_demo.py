...
@app.cell
def __(
    bayes_question, bayes_response, bayes_k, bayes_sigma,
    all_loadings, means, np,
):
    ...
    return (
        b_obs_q, b_r_obs, b_k, b_sigma_prior,
        b_loadings_k, b_L_obs, b_mu_obs,
        b_prior_mean, b_prior_cov,
        b_posterior_mean, b_posterior_cov,
    )
...
def __(
    ...
    # Panel 1: PRIOR
    _ax1 = _axes[0]
    _ax1.set_title("PRIOR\n(belief before observation)", fontsize=16, fontweight='bold', color=_CYBER_CYAN)
    if b_k >= 2:
        _confidence_ellipse(b_prior_mean, b_prior_cov, _ax1, n_std=1, edgecolor=_CYBER_CYAN, linewidth=2.5)
        _confidence_ellipse(b_prior_mean, b_prior_cov, _ax1, n_std=2, edgecolor=_CYBER_CYAN, linewidth=1.5, linestyle='--', alpha=0.6)
        _ax1.scatter([0], [0], color=_CYBER_CYAN, s=150, marker='+', linewidths=3, zorder=5)
        _ax1.set_xlabel("θ₁", fontsize=14); _ax1.set_ylabel("θ₂", fontsize=14)
    else:
        ...
        _ax1.set_xlabel("θ₁", fontsize=14); _ax1.set_ylabel("Density", fontsize=14)
    _ax1.set_xlim(-_plot_range, _plot_range)
    if b_k >= 2: _ax1.set_ylim(-_plot_range, _plot_range)
    ...
    _ax1.text(0.05, 0.95, f"θ ~ N(0, {b_sigma_prior:.1f}²·I)", transform=_ax1.transAxes,
             fontsize=13, va='top', family='monospace', color='white', fontweight='bold',
             bbox=dict(boxstyle='round', fc=_CYBER_BG, ec=_CYBER_CYAN, alpha=0.9, pad=0.5))

    # Panel 2: LIKELIHOOD
    _ax2 = _axes[1]
    _ax2.set_title("LIKELIHOOD\n(constraint from observation)", fontsize=16, fontweight='bold', color=_CYBER_YELLOW)
    ...
    if b_k >= 2:
        if abs(b_L_obs[1]) > 1e-6:
            ...
            _ax2.plot(_theta1[_valid], _theta2[_valid], color=_CYBER_YELLOW, linewidth=3.5)
        elif abs(b_L_obs[0]) > 1e-6:
            _ax2.axvline(x=_constraint_val / b_L_obs[0], color=_CYBER_YELLOW, linewidth=3.5)
        _arrow_scale = 1.8
        _ax2.arrow(0, 0, b_L_obs[0]*_arrow_scale, b_L_obs[1]*_arrow_scale,
                  head_width=0.2, head_length=0.12, fc=_CYBER_ORANGE, ec=_CYBER_ORANGE, lw=2.5, zorder=5)
        _ax2.text(b_L_obs[0]*_arrow_scale*1.15, b_L_obs[1]*_arrow_scale*1.15, 'Λ', fontsize=16,
                 color=_CYBER_ORANGE, fontweight='bold', ha='center')
        _ax2.set_xlabel("θ₁", fontsize=14); _ax2.set_ylabel("θ₂", fontsize=14)
    else:
        _theta_constrained = _constraint_val / (b_L_obs[0] + 1e-10)
        _ax2.axvline(x=_theta_constrained, color=_CYBER_YELLOW, linewidth=3.5)
        _ax2.set_xlabel("θ₁", fontsize=14); _ax2.set_ylim(0, 1)
    _ax2.set_xlim(-_plot_range, _plot_range)
    if b_k >= 2: _ax2.set_ylim(-_plot_range, _plot_range)
    ...
    _ax2.text(0.05, 0.95, f"r = {b_r_obs:.0f}\nΛ'θ + μ = r", transform=_ax2.transAxes,
             fontsize=13, va='top', family='monospace', color='white', fontweight='bold',
             bbox=dict(boxstyle='round', fc=_CYBER_BG, ec=_CYBER_YELLOW, alpha=0.9, pad=0.5))

    # Panel 3: POSTERIOR
    _ax3 = _axes[2]
    _ax3.set_title("POSTERIOR\n(updated belief)", fontsize=16, fontweight='bold', color=_CYBER_MAGENTA)
    if b_k >= 2:
        _confidence_ellipse(b_posterior_mean, b_posterior_cov, _ax3, n_std=1, edgecolor=_CYBER_MAGENTA, linewidth=2.5)
        _confidence_ellipse(b_posterior_mean, b_posterior_cov, _ax3, n_std=2, edgecolor=_CYBER_MAGENTA, linewidth=1.5, linestyle='--', alpha=0.6)
        _confidence_ellipse(b_prior_mean, b_prior_cov, _ax3, n_std=1, edgecolor=_CYBER_CYAN, linewidth=1, linestyle=':', alpha=0.4)
        _ax3.scatter([0], [0], color=_CYBER_CYAN, s=80, marker='+', linewidths=2, alpha=0.5)
        _ax3.scatter([b_posterior_mean[0]], [b_posterior_mean[1]], color=_CYBER_MAGENTA, s=120, marker='X', linewidths=2, zorder=5)
        _ax3.annotate('', xy=(b_posterior_mean[0], b_posterior_mean[1]), xytext=(0, 0),
                     arrowprops=dict(arrowstyle='->', color='#FF1493', lw=2.5))
        _ax3.set_xlabel("θ₁", fontsize=14); _ax3.set_ylabel("θ₂", fontsize=14)
    else:
        _x = np.linspace(-_plot_range, _plot_range, 200)
        _y_prior = np.exp(-0.5 * (_x - b_prior_mean[0])**2 / b_prior_cov[0, 0])
        _y_post = np.exp(-0.5 * (_x - b_posterior_mean[0])**2 / (b_posterior_cov[0, 0] + 1e-10))
        ...
        _ax3.set_xlabel("θ₁", fontsize=14); _ax3.set_ylabel("Density", fontsize=14)
    _ax3.set_xlim(-_plot_range, _plot_range)
    if b_k >= 2: _ax3.set_ylim(-_plot_range, _plot_range)
    ...
    _ax3.text(0.05, 0.95, "Prior × Likelihood\n= Posterior", transform=_ax3.transAxes,
             fontsize=13, va='top', family='monospace', color='white', fontweight='bold',
...
def __(
    ...
    _ax.set_facecolor(_CYBER_BG)
    _bars = _ax.bar(_responses, _probs, color=_CYBER_CYAN, alpha=0.8, edgecolor='white', linewidth=2)
    _ax.axvline(x=_pred_mean, color=_CYBER_GREEN, linewidth=3, linestyle='--', label=f'E[r]={_pred_mean:.2f}')

    for _bar, _p in zip(_bars, _probs):
        _ax.text(_bar.get_x() + _bar.get_width()/2, _bar.get_height() + 0.01,
                f'{_p:.0%}', ha='center', va='bottom', color='white', fontsize=11, fontweight='bold')

    _ax.set_xlabel("Partner's Response", fontsize=12, color='white')
    _ax.set_ylabel("Probability", fontsize=12, color='white')
    _ax.set_title(f"Bayesian: P(response on Q{b_target_q} | observed Q{b_obs_q}={b_r_obs:.0f})",
    ...
    _ax.legend(loc='upper right', fontsize=10, facecolor=_CYBER_BG, edgecolor=_CYBER_GRID, labelcolor='white')
    ...
    _domain_note = "**Same domain** → stronger transfer" if _same_domain else "**Different domain** → weaker transfer"
...
def __(mo, questions, domain_labels, sample_participants):
    ...
    ego_tau = mo.ui.slider(
        start=0.5, stop=4.0, step=0.5, value=2.0,
...
@app.cell
def __(
    ego_question, ego_response, ego_profile, ego_tau,
    sample_participants, np,
):
    ...
    e_predictions = np.clip(_base_rate + e_perceived_sim * e_self_sim * _projection_weight, 0.01, 0.99)

    return (
        e_obs_q, e_r_obs, e_r_self, e_tau,
        e_r_self_obs, e_perceived_sim, e_self_sim,
        e_predictions,
    )
...
def __(
    ...
    ego_question, ego_response, ego_profile, ego_tau,
    e_obs_q, e_r_obs, e_r_self, e_tau,
    e_r_self_obs, e_perceived_sim, e_self_sim, e_predictions,
    questions, domain_labels, DOMAIN_RANGES,
    np, plt,
):
    _CYBER_BG = '#0D0221'
    _CYBER_GRID = '#1a1a3a'
    ...
    _ax1.bar([e_obs_q], [e_r_self[e_obs_q]], color=_CYBER_YELLOW, edgecolor='white', linewidth=2)
    _ax1.axhline(y=e_r_self_obs, color=_CYBER_YELLOW, linestyle='--', linewidth=2, alpha=0.7, label=f'Your obs response: {e_r_self_obs:.1f}')
    for _domain, (_start, _end) in sorted(DOMAIN_RANGES.items(), key=lambda x: x[1][0]):
        ...
        _ax1.text((_start + _end) / 2 - 0.5, 0.3, _domain[:3].upper(), ha='center', fontsize=8, color='white', alpha=0.7)
    _ax1.set_ylabel("Your Response", color='white', fontsize=12)
    _ax1.set_title(f"YOUR RESPONSES ({ego_profile.value}) — Yellow = observed question",
    ...
    _ax2.bar([e_obs_q], [e_self_sim[e_obs_q]], color=_CYBER_ORANGE, edgecolor='white', linewidth=2)
    for _domain, (_start, _end) in sorted(DOMAIN_RANGES.items(), key=lambda x: x[1][0]):
        if _start > 0:
            _ax2.axvline(x=_start - 0.5, color=_CYBER_GRID, alpha=0.8, linewidth=1)
    _ax2.set_ylabel("Self-Similarity", color='white', fontsize=12)
    _ax2.set_title(f"SELF-RESPONSE SIMILARITY: exp(-|s_q - s_obs| / τ) where τ={e_tau:.1f}",
                  fontsize=14, fontweight='bold', color=_CYBER_ORANGE)
    _ax2.set_ylim(0, 1.1)
    ...
    _ax3.bar(_x, e_predictions, color=_colors, alpha=0.8, edgecolor='white', linewidth=0.5)
    _ax3.bar([e_obs_q], [e_predictions[e_obs_q]], color=_CYBER_GREEN, edgecolor='white', linewidth=2)
    for _domain, (_start, _end) in sorted(DOMAIN_RANGES.items(), key=lambda x: x[1][0]):
        if _start > 0:
    ...
    _ax3.set_title(f"EGOCENTRIC PREDICTION: P(commonality) = base + perceived_sim × self_sim × weight",
                  fontsize=14, fontweight='bold', color=_CYBER_GREEN)
    _ax3.set_ylim(0, 1.0)
...
def __(
    ...
    _ax.set_facecolor(_CYBER_BG)
    _bars = _ax.bar(_responses, _probs, color=_CYBER_YELLOW, alpha=0.8, edgecolor='white', linewidth=2)
    _ax.axvline(x=_self_response, color=_CYBER_GREEN, linewidth=3, linestyle='--', label=f'Your r={_self_response:.1f}')
    ...
    _ax.set_xlabel("Partner's Response", fontsize=12, color='white')
    _ax.set_ylabel("Probability", fontsize=12, color='white')
    _ax.set_title(f"Egocentric: P(response on Q{e_target_q} | observed Q{e_obs_q}={e_r_obs:.0f})",
                 fontsize=14, fontweight='bold', color=_CYBER_YELLOW)
    _ax.set_xticks([1, 2, 3, 4, 5])
    _ax.set_ylim(0, max(_probs) * 1.4)
    _ax.tick_params(colors='white', labelsize=11)
...
@app.cell
def __(
    cmp_question, cmp_response, cmp_lambda, cmp_profile, cmp_k,
    sample_participants, all_loadings, means, np,
):
    # Compute both models
    ...
    c_bayes_vars = np.array([c_loadings[i] @ c_post_cov @ c_loadings[i] for i in range(35)])
    ...
    return (
        c_obs_q, c_r_obs, c_lambda, c_r_self, c_k,
        c_loadings, c_post_mean, c_post_cov,
        c_bayes_means, c_bayes_vars,
        c_r_self_obs, c_perceived_sim, c_ego_preds,
    )
...
def __(
    ...
    cmp_question, cmp_response, cmp_lambda, cmp_profile, cmp_k,
    c_obs_q, c_r_obs, c_lambda, c_r_self, c_k,
    c_bayes_means, c_bayes_vars, c_ego_preds,
    c_r_self_obs, c_perceived_sim,
    means, domain_labels, DOMAIN_RANGES,
    np, plt,
):
    _CYBER_BG = '#0D0221'
    _CYBER_GRID = '#1a1a3a'
    ...
    _ax1.bar(_x, c_bayes_means, yerr=_bayes_stds, color=_colors, alpha=0.8, capsize=2,
            error_kw={'linewidth': 1, 'alpha': 0.6, 'color': 'white'}, edgecolor='white', linewidth=0.5)
    _ax1.bar([c_obs_q], [c_bayes_means[c_obs_q]], color=_CYBER_CYAN, edgecolor='white', linewidth=2)
    ...
    _ax1.set_title(f"BAYESIAN (k={c_k}) — Population covariance structure",
                  fontsize=14, fontweight='bold', color=_CYBER_CYAN)
    _ax1.set_ylim(0, 6)
    _ax1.set_xlim(-0.5, 34.5)
    _ax1.tick_params(colors='white')
    ...
    _ax2.bar([c_obs_q], [_ego_scaled[c_obs_q]], color=_CYBER_YELLOW, edgecolor='white', linewidth=2)
    _ax2.scatter(_x, c_r_self, color='white', s=30, marker='o', alpha=0.6, label='Your responses', zorder=3)
    for _domain, (_start, _end) in sorted(DOMAIN_RANGES.items(), key=lambda x: x[1][0]):
        if _start > 0:
            _ax2.axvline(x=_start - 0.5, color=_CYBER_GRID, alpha=0.8, linewidth=1)
    _ax2.set_xlabel("Question", color='white', fontsize=12)
    _ax2.set_ylabel("P(commonality) scaled", color='white', fontsize=12)
    _ax2.set_title(f"EGOCENTRIC ({cmp_profile.value}) — Your belief structure",
                  fontsize=14, fontweight='bold', color=_CYBER_YELLOW)
    _ax2.set_ylim(0, 6)
    _ax2.set_xlim(-0.5, 34.5)
    _ax2.tick_params(colors='white')
...
def __(mix_lambda, np, plt, mo):
    ...
    _human_gradient = 0.124  # (same_shared - same_opposing) - (diff_shared - diff_opposing)
    ...
    _ax1 = _axes[0]
    _ax1.set_facecolor(_CYBER_BG)
    _ax1.plot(_lambda_vals, _model_grads, color=_CYBER_MAGENTA, linewidth=3, label='Model gradient')
    _ax1.axhline(y=_human_gradient, color='white', linestyle='--', linewidth=2, label=f'Human gradient ({_human_gradient:.3f})')
    _ax1.axhline(y=0, color=_CYBER_GRID, linestyle='-', linewidth=1)

    # Mark current λ
    _ax1.scatter([_curr_lambda], [_curr_grad], color=_CYBER_GREEN, s=200, zorder=5, edgecolor='white', linewidth=2)
    _ax1.axvline(x=_curr_lambda, color=_CYBER_GREEN, linestyle=':', linewidth=2, alpha=0.7)

    # Mark endpoints
    _ax1.scatter([0], [_bayes_grad], color=_CYBER_CYAN, s=150, zorder=4, marker='s', label='Bayesian (λ=0)')
    _ax1.scatter([1], [_ego_grad], color=_CYBER_YELLOW, s=150, zorder=4, marker='s', label='Egocentric (λ=1)')

    _ax1.set_xlabel("λ (mixture weight)", fontsize=14, color='white')
    _ax1.set_ylabel("Generalization Gradient", fontsize=14, color='white')
    _ax1.set_title("Model Gradient vs Mixture Weight", fontsize=16, fontweight='bold', color='white')
    ...
    _bar_colors = [_CYBER_CYAN if i == 0 else _CYBER_YELLOW if i == 2 else _CYBER_MAGENTA
                   for i in range(3)]
    _bar_vals = [95, _curr_pct, 12]
    _bar_labels = ['Bayesian\n(λ=0)', f'Current\n(λ={_curr_lambda:.2f})', 'Egocentric\n(λ=1)']

    _bars = _ax2.bar(_bar_labels, _bar_vals, color=_bar_colors, alpha=0.8, edgecolor='white', linewidth=2)

    # Add percentage labels
    for _bar, _val in zip(_bars, _bar_vals):
        _ax2.text(_bar.get_x() + _bar.get_width()/2, _bar.get_height() + 2,
                 f'{_val:.0f}%', ha='center', va='bottom', color='white', fontsize=14, fontweight='bold')

    _ax2.axhline(y=100, color='white', linestyle='--', linewidth=2, alpha=0.5, label='Human (100%)')
    _ax2.set_ylabel("% of Human Gradient Captured", fontsize=14, color='white')
    _ax2.set_title("Model Fit to Human Data", fontsize=16, fontweight='bold', color='white')
    _ax2.set_ylim(0, 120)
