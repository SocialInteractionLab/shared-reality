...
def create_chat_timecourse_batch(
    prompt_type: str = "pshared",
    bin_seconds: int = 15,
    max_bins: int = 13,
    ...
    batch_requests = []
    prompt_fn = (
        create_pshared_prompt if prompt_type == "pshared" else create_chat_prompt
    )
...
def compute_pshared_metrics(results: list, tau: int = 2) -> pd.DataFrame:
    ...
    for result in results:
        ...
        for q_str, pred_agreement in agreement_probs.items():
            ...
            q_domain = (
                questions.iloc[q_idx]["domain"] if q_idx < len(questions) else None
            )
            ...
            if q_num == matched_q:
                ...
                q_domain
                and matched_domain
                and q_domain.lower() == matched_domain.lower()
            ):
...
def cmd_download(args):
    ...
    if args.type == "pshared":
        ...
        raise NotImplementedError(
            "Use 'pshared' type. Chat parsing not implemented in unified pipeline."
        )
...
def main():
    ...
    sub.add_argument(
        "--tau", type=int, default=2, help="Agreement threshold (default: 2)"
    )
    ...
    sub.add_argument("type", choices=["pshared", "chat"], help="Experiment type")
    sub.add_argument(
        "--version", default="", help='Version suffix for output file (e.g., "tau1")'
    )
    sub.add_argument(
        "--tau",
        type=int,
        default=2,
        help="Agreement threshold for ground truth (default: 2)",
    )
