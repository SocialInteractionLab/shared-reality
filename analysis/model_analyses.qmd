...
# Human effects
human_effects = [HUMAN_RATES[(qt, 'high')] - HUMAN_RATES[(qt, 'low')] for qt in qt_keys]

# Model effects
model_effects = []
for qt in qt_keys:
    qt_preds = main_preds[main_preds['question_type'] == qt]
    high = qt_preds[qt_preds['match_type'] == 'high']['pred_prob'].mean()
    low = qt_preds[qt_preds['match_type'] == 'low']['pred_prob'].mean()
    model_effects.append(high - low)

ax.bar(x - width/2, human_effects, width, label='Human', color='#2ecc71', alpha=0.8)
ax.bar(x + width/2, model_effects, width, label='Model (k=4)', color='#3498db', alpha=0.8)
ax.set_xticks(x)
