...
# Human rates for comparison (using stance from responses.csv)
chat_data = data[data['experiment'] == 'chat'].copy()
human_rates_by_stance = {}
for stance in ['shared', 'opposing']:
    for qt in ['observed', 'same_domain', 'different_domain']:
        ...
        human_rates_by_stance[(stance, qt)] = subset['participant_binary_prediction'].mean()

# Map to LLM match_type names for plotting
human_rates = {('high', qt): human_rates_by_stance[('shared', qt)] for qt in ['observed', 'same_domain', 'different_domain']}
human_rates.update({('low', qt): human_rates_by_stance[('opposing', qt)] for qt in ['observed', 'same_domain', 'different_domain']})
