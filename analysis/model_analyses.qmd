...
This document reproduces the main computational model analyses (Figures 5-6).

- **Section 1**: Setup and data loading
- **Section 2**: Parameter fitting (grid search)
- **Section 3**: Figure 5 (Bayesian model predictions)
- **Section 4**: Figure 6 (LLM analysis)
...
## 3. Figure 5: Bayesian Model Captures Structured Generalization
...
for i, stance in enumerate(['opposing', 'shared']):
    offset = -width/2 if stance == 'opposing' else width/2
    rates = [model_rates[(qt, stance)] for qt in qt_keys]
    yerr = [[model_rates[(qt, stance)] - rate_cis[(qt, stance)][0] for qt in qt_keys],
            [rate_cis[(qt, stance)][1] - model_rates[(qt, stance)] for qt in qt_keys]]
    ax.bar(x + offset, rates, width, color=COLORS[stance], label='Opposing Stance' if stance == 'opposing' else 'Shared Stance',
           yerr=yerr, capsize=0, error_kw={'linewidth': 1.5})

ax.set_xticks(x)
ax.set_xticklabels(qt_labels)
ax.set_ylabel('Model Expected Commonality', fontweight='bold')
ax.set_ylim(0, 1.0)
...
for domain in DOMAIN_RANGES.keys():
    for qt in ['same_domain', 'different_domain']:
        ...
        if len(subset) == 0:
            continue
        shared = subset[subset['stance'] == 'shared']
        opposing = subset[subset['stance'] == 'opposing']
        if len(shared) == 0 or len(opposing) == 0:
            continue
        domain_data.append({
            'domain': domain,
            'question_type': qt,
            'model_effect': shared['pred_prob'].mean() - opposing['pred_prob'].mean(),
            'human_effect': shared['actual'].mean() - opposing['actual'].mean()
        })
...
for qt in ['same_domain', 'different_domain']:
    ...
    color = COLORS['same'] if qt == 'same_domain' else COLORS['diff']
    marker = '*' if qt == 'same_domain' else 'o'
    size = 220 if qt == 'same_domain' else 80
    label = 'Same Domain' if qt == 'same_domain' else 'Different Domain'
    ax.scatter(subset['model_effect'], subset['human_effect'],
               c=color, marker=marker, s=size, edgecolors='white', linewidth=0.5, label=label)
...
# Check robustness: correlation excluding religion same_domain outlier
excl = domain_df[~((domain_df['domain'] == 'religion') & (domain_df['question_type'] == 'same_domain'))]
r_excl = np.corrcoef(excl['model_effect'], excl['human_effect'])[0, 1]
print(f"Correlation with all points: r = {r:.3f} (n={len(domain_df)})")
print(f"Correlation excluding religion same_domain: r = {r_excl:.3f} (n={len(excl)})")
...
ax.set_xlabel('Model Transfer Effect', fontweight='bold')
ax.set_ylabel('Human Transfer Effect', fontweight='bold')
...
ax.set_ylabel('Gradient (Same - Different)', fontweight='bold')
...
```{python}
#| label: fig4-combined
#| fig-cap: "Figure 5: Factor structure captures systematic generalization"
#| fig-width: 12
#| fig-height: 4
...
ax.set_xlabel('Model Transfer Effect', fontweight='bold')
ax.set_ylabel('Human Transfer Effect', fontweight='bold')
ax.legend(frameon=True, loc='lower right', edgecolor='black', facecolor='white', framealpha=1)
...
### Expected Commonality Timecourse

```{python}
#| label: fig5
#| fig-cap: "Figure 6: LLM recovers structured generalization from conversation transcripts"
#| fig-width: 9
#| fig-height: 4

# Load timecourse data
pshared_df = pd.read_csv(base_dir / 'data' / 'llm_results' / 'pshared_timecourse.csv')
pshared_hl = pshared_df[pshared_df['stance'].isin(['shared', 'opposing'])].copy()

# Compute means
timecourse = pshared_hl.groupby(
    ['time_bin', 'bin_seconds', 'stance', 'question_category']
...
for stance in ['shared', 'opposing']:
    for qt in ['observed', 'same_domain', 'different_domain']:
        ...
        human_rates[(stance, qt)] = subset['participant_binary_prediction'].mean()

# Plot
fig, axes = plt.subplots(1, 2, figsize=(9, 4), sharey=True)

cat_styles = {
    'matched': {'color': COLORS['focal'], 'lw': 2.5, 'marker': 'o', 'ms': 5, 'label': 'Focal Topic'},
...
# No-chat baseline: mean expected commonality per stance (across all question types)
nochat_baseline = {}
nochat_data_fig = data[data['experiment'] == 'no-chat'].copy()
for stance in ['shared', 'opposing']:
    subset = nochat_data_fig[nochat_data_fig['stance'] == stance]
    nochat_baseline[stance] = subset['participant_binary_prediction'].mean()

for ax, (stance, title) in zip(axes, [('opposing', 'Opposing Stance'), ('shared', 'Shared Stance')]):
    for cat in ['matched', 'same_domain', 'different_domain']:
        subset = timecourse[(timecourse['stance'] == stance) & (timecourse['question_category'] == cat)]
        ...
        # Human dashed line
        ax.axhline(human_rates[(stance, cat_to_human[cat])], color=style['color'],
                   linestyle='--', linewidth=1.5, alpha=0.7)

    # No-chat baseline
    ax.axhline(nochat_baseline[stance], color='black',
               linestyle=':', linewidth=1.5, alpha=0.8, label='Imagined Interaction Baseline')

    ax.set_xlabel('Conversation Time (s)')
    ax.set_title(title, fontweight='bold', color=COLORS[stance])
    ax.set_ylim(0.1, 1.0)
    ax.set_xlim(-5, 195)
...
plt.tight_layout()
plt.savefig(base_dir / 'outputs' / 'figures' / 'figure6.pdf', dpi=300, bbox_inches='tight')
plt.show()
```
...
```{python}
#| label: summary
print('=== Key Results (imagined interaction condition) ===\n')
