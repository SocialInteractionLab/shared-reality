...
print(f"Model (ground truth input): {model_gradient_groundtruth:+.4f} ({100*model_gradient_groundtruth/human_gradient:.0f}% of human)")
print(f"Model (perceived input):    {model_gradient_perceived:+.4f} ({100*model_gradient_perceived/human_gradient:.0f}% of human)")
...
for i, (h_grad, m_grad, color, n) in enumerate(zip(human_grads, model_grads_perc, error_colors, ns)):
    if np.isnan(h_grad):
        continue
    # Human bar
    ax.bar(i - bar_width/2, h_grad, bar_width, color='#2c3e50', alpha=0.8,
           label='Human' if i == 0 else '')
    # Model bar (using perceived input)
    ...
    ax.text(i + bar_width/2, m_grad + 0.01, f'{pct:.0f}%', ha='center', fontsize=10, fontweight='bold')
